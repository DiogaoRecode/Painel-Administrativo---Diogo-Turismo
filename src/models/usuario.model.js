...
DataSchema.methods.isCorrectPassword = function (password,callback){ //Para conferir se a senha está correta
  bcrypt.compare(password,this.senha_usuario,(err,same)=>{
    if(err){
      callback(err);
    }else{
      callback(err, same);
