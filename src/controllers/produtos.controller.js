...
module.exports = {
  async index(req,res){  
    const product = await Produto.find(); //find: Encontrar todos os Produtos
    res.json(product);
  },
  ...
  async details(req,res){  
    ...
    const product = await Produto.findOne({_id}); //findOne: encontrar apenas um Produto
    res.json(product);
  },
...
async update(req,res){
  ...
  const product = await Produto.findOneAndUpdate({_id},data,{new:true}); //new:treu ->vai salvar os dados da nova aquisição
  res.json(product);
},
